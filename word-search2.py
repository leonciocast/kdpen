    ...
    while not placed:
        if orientation == 0:  # Horizontal
            ...
            if reverse:
                word = word[::-1]
            space_available = all(
                board[row][c] == "-" or board[row][c] == word[i]
            ...
            if reverse:
                word = word[::-1]
            space_available = all(
                board[r][col] == "-" or board[r][col] == word[i]
            ...
            space_available = all(
                ...
                for i, (r, c) in enumerate(
                    zip(range(row, row + len(word)), range(col, col + len(word)))
                )
            )
            if space_available:
                for i, (r, c) in enumerate(
            ...
            space_available = all(
                board[r][c] == "-" or board[r][c] == word[i]
                for i, (r, c) in enumerate(
                    zip(range(row, row - len(word), -1), range(col, col + len(word)))
                )
            )
            if space_available:
                for i, (r, c) in enumerate(
